...
echo "   Then we'll query each open port for its reported service name."
echo
...
# Dynamic port-to-service map
declare -A port_services
for port in "${open_ports[@]}"; do
    # Use curl to grab X-Service-Name header
    service_name=$(curl -s -o /dev/null -D - "http://localhost:$port" | grep -i "X-Service-Name" | awk -F: '{gsub(/^[ \t]+/, "", $2); print $2}')
    port_services["$port"]="${service_name:-unknown}"
done
